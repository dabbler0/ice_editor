...
(function() {
        ...
        if (char === '%') {
          ...
          if (char === 't') {
            ...
            socket.onkeyup = function() {
              ...
              return ($("#gen")).text(formatLine(root._ice_tree));
            };
          } else {
      ...
      if (in_special) {
        if (char === '%') {
          ...
          if (char === 't') {
            ...
            socket._ice_number = arg_number;
            if ((fargs[arg_number] != null) && fargs[arg_number].length > 0) {
            ...
            input_socket = document.createElement("input");
            input_socket.className = "input_socket";
            input_socket.value = (tree.args[arg_number] != null) && tree.args[arg_number].type === "textonly" ? tree.args[arg_number].form : "";
            input_socket._ice_number = arg_number;
            input_socket.onkeyup = function() {
              return tree.args[this._ice_number].form = this.value;
            };
  ...
  window.onload = function() {
    var details, dialog, dialog_title, list, palette, section, section_name, template, templates, workspace, _i, _len;
    root = makeElement("(->\n%w\n)()");
    palette = $("#palette");
    workspace = $("#workspace");
    ($("#trashbin")).droppable({
    ...
    templates = {
      "dialogs": ["alert(%v)", "prompt(%v)", "confirm(%v)"],
      "control structures": ["for %t in [1..%v]\n%w", "for %t in %v\n%w", "for %t of %v\n%w", "if %v\n%w", "if %v\n%w\nelse\n%w", "break"],
      "turtle": ["fd(%v)", "bk(%v)", "rt(%v)", "lt(%v)", "pen(%v)", "dot(%v,%v)"],
      "arrays": ["[]", "[%v..%v]", "%v[%v]", "%v.push(%v)", "%v.splice(%v, %v)", "%v[%v..%v]"],
      "variables": ["%t = %v", "%t"],
      "strings": ["\"%t\"", "%v[%v..%v]"],
      "logic": ["(%v and %v)", "(%v or %v)", "true", "false"],
      "comparisons": ["(%v == %v)", "(%v > %v)", "(%v < %v)", "(%v >= %v)", "(%v <= %v)"],
      "math": ["(%v + %v)", "(%v - %v)", "(%v * %v)", "(%v / %v)", "(%v %% %v)", "Math.pow(%v, %v)", "Math.sin(%v)", "Math.cos(%v)", "Math.tan(%v)"],
      "functions": ["((%t) ->\n%w)", "%t = (%t) ->\n%w", "return %v", "%v(%v)"]
    };
    for (section_name in templates) {
    ...
    ($("#run")).click(function() {
      return out_iframe.CoffeeScript["eval"](formatLine(root._ice_tree));
    });
    ($("#reset")).click(function() {
      return out_iframe.location.reload();
    });
    ...
    $("#load").click(function() {
      ...
      for (key in localStorage) {
        _fn();
        _results.push($("#name").unbind("keydown").keydown(function(e) {
          if (e.keyCode === 13) {
            $(root).remove();
