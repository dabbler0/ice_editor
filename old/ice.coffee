      ...
      if char == '%'
        ...
        if char == 't'
          ...
          socket.onkeyup = ->
            ...
            #EXAMPLE ONLY
            ($ "#gen").text formatLine root._ice_tree
    ...
    if in_special
      if char == '%'
        ...
        if char == 't'
          ...
          if fargs[arg_number]? and fargs[arg_number].length > 0
            ...
            for el in fargs[arg_number]
              ...
              el._ice_insert_type = "block_socket"
              new_div = document.createElement "div"
              new_div.appendChild el
              socket.appendChild new_div
          else
            console.log "block_socket", socket, "is ice_insertable"
            socket._ice_insertable = true
        else
          input_socket = document.createElement "input"
          input_socket.className = "input_socket"
          input_socket.value = if tree.args[arg_number]? and tree.args[arg_number].type == "textonly" then tree.args[arg_number].form else ""
          ...
          if fargs[arg_number] != undefined and fargs[arg_number]?
            ...
            fargs[arg_number]._ice_parent = tree
            fargs[arg_number]._ice_number = arg_number
            fargs[arg_number]._ice_literal_parent = socket
            fargs[arg_number]._ice_insertable = false
            $(input_socket).hide()
            socket._ice_insertable = false
          else
            socket._ice_insertable = true
...
window.onload = ->
  ...
  for section_name, section of templates
    details = $("<details>").append($("<summary>").text(section_name))
    palette.append details
    for template in section
      details.append $("<div>").addClass("template_wrapper").append makeTemplateElement template
  ...
  ($ "#run").click(->
    out_iframe.CoffeeScript.eval formatLine root._ice_tree
  )
  
  ($ "#reset").click(->
    out_iframe.location.reload()
  )
